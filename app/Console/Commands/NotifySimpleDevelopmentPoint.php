...
class NotifySimpleDevelopmentPoint extends Command
{
    ...
    {
        $this->notifySimpleDevelopPoint();
    }

    private function notifySimpleDevelopPoint()
    {
        ...
        $mainPayloads = $this->generateSimpleMainDevelopPointPayload($targetPoint, $donePoint);
        $memberPointPayloads = $this->generateSimpleDevelopPointPayload($memberTasks);
    ...
    private function generateSimpleMainDevelopPointPayload(float $targetPoint, float $donePoint): array
    {
        $mainTemplate = json_encode(config('slack.template.developPoint.simpleMain'));
    ...
    private function generateSimpleDevelopPointPayload(Collection $memberTasks): array
    {
        return collect($memberTasks)
            ->flatMap(function ($member): array {
                $memberTemplate = json_encode(config('slack.template.developPoint.simpleMember'));
